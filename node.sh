...
LOGLEVEL="info"
# Set dedicated home directory for the vectord instance
HOMEDIR="$HOME/.vectord"
...
while [[ $# -gt 0 ]]; do
	...
	-n)
		...
		;;
	--no-install)
		echo "Flag --no-install passed -> Skipping installation of the vectord binary."
		install=false
		shift # Move past the flag
		;;
	--remote-debugging)
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	vectord config set client chain-id "$CHAINID" --home "$HOMEDIR"
	vectord config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	echo "$VAL_MNEMONIC" | vectord keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER1_MNEMONIC" | vectord keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | vectord keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | vectord keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | vectord keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	...
	## 3. Clone this ~/.vectord home directory into some others, let's say `~/.clonedOsd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedOsd/config/gentx/` folders into the original `~/.vectord/config/gentx`

	# Collect genesis tx
	vectord genesis collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	vectord genesis validate-genesis --home "$HOMEDIR"
