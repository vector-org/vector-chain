...
HTTPS_GIT := github.com/rollchains/vectorchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=vectorchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=vectord \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/vectord ./cmd/vectord
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/vectord.exe ./cmd/vectord
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/vectord
...
draw-deps:
	...
	@goviz -i ./cmd/vectord -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c vectorchain --local -f chains.yaml
endif
...
set-testnet-configs:
	vectord config set client chain-id localchain_9000-1
	vectord config set client keyring-backend test
	vectord config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | vectord keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | vectord keys add acc1 --recover`
