...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["staking"]["params"]["bond_denom"]="avctr"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="avctr"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="avctr"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["params"]["expedited_min_deposit"][0]["denom"]="avctr"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["evm"]["params"]["evm_denom"]="avctr"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["mint"]["params"]["mint_denom"]="avctr"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["globalfee"]["params"]["minimum_gas_prices"]=[{"denom":"avctr","amount":"0.0001"}]' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

	# Add default token metadata to genesis
	jq '.app_state["bank"]["denom_metadata"]=[{"description":"Vector Token for staking and gas fees.","denom_units":[{"denom":"avctr","exponent":0,"aliases":["atto-vector"]},{"denom":"vctr","exponent":18,"aliases":[]}],"base":"avctr","display":"vctr","name":"Vector Token","symbol":"VCTR","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"avctr",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	evmd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000avctr --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER1_KEY" 1000000000000000000000avctr --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER2_KEY" 1000000000000000000000avctr --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER3_KEY" 1000000000000000000000avctr --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmd genesis add-genesis-account "$USER4_KEY" 1000000000000000000000avctr --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	evmd genesis gentx "$VAL_KEY" 1000000000000000000000avctr --gas-prices ${BASEFEE}avctr --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
...
evmd start "$TRACE" \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0.0001avctr \
	--home "$HOMEDIR" \
